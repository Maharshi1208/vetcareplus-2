...
export default function VetsPage() {
  ...
  useEffect(() => {
    ...
    if (s) {
      setFlash(s);
      // clear once so it doesn't re-appear on refresh/back
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Vets</h1>
          <p className="text-sm text-gray-500">Manage veterinarian profiles (UI-only).</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="inline-flex items-center rounded-full border px-3 py-1 text-sm text-gray-700">
            Total: <span className="ml-1 font-semibold">{vets.length}</span>
          </span>
          <Link
            to="/vets/add"
